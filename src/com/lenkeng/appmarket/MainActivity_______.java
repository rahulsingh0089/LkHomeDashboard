...
/*
 * $Id: MainActivity.java 62 2014-01-21 12:29:04Z xgh $
 */
public class MainActivity_______ extends Activity implements OnFocusChangeListener,
	...
	private static int barType=0; //栏目类别
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Intent it=getIntent();
		barType=it.getIntExtra("flag", 0);
		
		map_movie.clear();
		map_game.clear();

		String host = URLs.getMarketHost();
		Log.i("ken", "--1001---host=" + host + " -MainActivity.onCreate()");
		Log.i("ken", "--1002---URL_API_HOST=" + URLs.getURL_API_HOST()
				+ " -MainActivity.onCreate()");
		Log.i("ken", "--1003---URLs.BASE=" + URLs.getBASE()
				+ " -MainActivity.onCreate()");
	...
	private void showThePage(int curPage) {
		View currentView=view_movie;
		//body.setCurrentItem(curPage, false);
		switch (curPage) {
		case 0:
			initMovieLayout();
			currentView=view_movie;
			
			view_movie.setVisibility(View.VISIBLE);
			view_app.setVisibility(View.GONE);
			view_game.setVisibility(View.GONE);
			view_search.setVisibility(View.GONE);
			
			view_movie.setFocusable(true);
			view_app.setFocusable(false);
			view_game.setFocusable(false);
			view_search.setFocusable(false);
			
			break;
		case 1:
			initAppLayout();
			currentView=view_app;
			
			view_app.setVisibility(View.VISIBLE);
			view_movie.setVisibility(View.GONE);
			view_game.setVisibility(View.GONE);
			view_search.setVisibility(View.GONE);
			
			view_app.setFocusable(true);
			view_movie.setFocusable(false);
			view_game.setFocusable(false);
			view_search.setFocusable(false);
		...
		case 2:
			...
			view_game.setVisibility(View.VISIBLE);
			view_app.setVisibility(View.GONE);
			view_movie.setVisibility(View.GONE);
			view_search.setVisibility(View.GONE);
			
			view_game.setFocusable(true);
			view_app.setFocusable(false);
			view_movie.setFocusable(false);
			view_search.setFocusable(false);
		...
		Log.e(TAG, "~~~~~~~~~~curpage="+curPage+",显示的view="+currentView);
		
		fl_body.bringChildToFront(currentView);
	}
	...
	private void initWidget() {
		...
		fl_body.addView(view_movie);
		fl_body.addView(view_app);
		fl_body.addView(view_game);
		fl_body.addView(view_search);
